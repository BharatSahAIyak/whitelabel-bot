...
const MessageItem: FC<MessageItemPropType> = ({ message }) => {
    ...
    const feedbackHandler = useCallback(
        ({ like, msgId }: { like: 0 | 1 | -1; msgId: string }) => {
            ...
            if (reaction !== 0)
                return toast.error('Cannot give feedback again');
            if (reaction === 0) {
                setReaction(like);
                                ...
                                if (optionDisabled) {
                                    toast.error(
                                        `${
                                            isWeather
                                                ? t(
                                                      'message.wait_before_choosing'
                                                  )
                                                : t(
                                                      'message.cannot_answer_again'
                                                  )
                                        }`
                                    );
                                } else {
                        ...
                        content?.data?.position === 'left' && (
                            ...
                            >
                                ...
                                {config?.allowFeedback &&
                                    (!content?.data?.isGuided ||
                                        content?.data?.card) && (
                                        <div className={styles.msgFeedback}>
                                            <div
                                                className={
                                                    styles.msgFeedbackIcons
                                                }
                                                style={{
                                                    border: `1px solid ${theme?.primary?.main}`,
                                                }}
                    ...
                    {content?.data?.choices?.choices?.length > 0 && (
                        <>
                            <Popup
                                ...
                                isCollapsed={
                                    content?.data?.choices?.isCollapsed ?? false
                                }
                                height={'20vh'}
                                onClose={() => {
                                ...
                                popupActive && (
                                    ...
                                    >
                                        <input
                                            placeholder={
                                                t(
                                                    'label.buttons_search_placeholder'
                                                ) || 'Search'
                                            }
                                            value={searchQuery}
                                            style={{
                ...
                >
                    ...
                    >
                        ...
                        >
                            {
                                <JsonToTable
                                    json={parseWeatherJson(
                                        JSON.parse(content?.text)?.table
                                    )}
                                />
                            }
                            <style>
